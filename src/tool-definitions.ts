...
export function buildToolDefinitions(godotBridgePort: number): MCPToolDefinition[] {
  return [
        ...
        // ==================== SCENE 2D TOOLS (Phase 2b) ====================
        {
          name: 'add_sprite_2d',
          description: 'Creates a Sprite2D with texture, optional region, and hframes/vframes. Use to add 2D sprites to a scene.',
          inputSchema: {
            type: 'object',
            properties: {
              projectPath: { type: 'string', description: 'Absolute path to project directory containing project.godot. Use the same path across all tool calls in a workflow.' },
              scenePath: { type: 'string', description: 'Path to the scene file' },
              parentNodePath: { type: 'string', description: 'Path to the parent node' },
              nodeName: { type: 'string', description: 'Name for the Sprite2D node' },
              texture: { type: 'string', description: 'Resource path to the texture image file' },
              position: { type: 'object', properties: { x: { type: 'number' }, y: { type: 'number' } }, description: 'Node position as Vector2' },
              rotation: { type: 'number', description: 'Rotation in degrees' },
              scale: { type: 'object', properties: { x: { type: 'number' }, y: { type: 'number' } }, description: 'Node scale as Vector2' },
              region: { type: 'object', properties: { x: { type: 'number' }, y: { type: 'number' }, width: { type: 'number' }, height: { type: 'number' } }, description: 'Region rect for sprite sheet' },
              hframes: { type: 'number', description: 'Horizontal frames (for animated sprites)' },
              vframes: { type: 'number', description: 'Vertical frames (for animated sprites)' },
              frames: { type: 'number', description: 'Total frames (deprecated, use hframes/vframes)' },
            },
            required: ['projectPath', 'scenePath', 'parentNodePath', 'nodeName'],
          },
        },
        {
          ...
          description: 'Creates a Camera2D node with optional follow target, zoom, limits, and smoothing. Use to add or configure the main camera in a 2D scene.',
          inputSchema: {
            type: 'object',
            properties: {
              projectPath: { type: 'string', description: 'Absolute path to project directory containing project.godot. Use the same path across all tool calls in a workflow.' },
              ...
              smoothing: { type: 'number', description: 'Position smoothing speed (0 to disable)' },
            },
            required: ['projectPath', 'scenePath', 'parentNodePath', 'nodeName'],
          },
        },
        {
          name: 'add_canvas_layer',
          description: 'Creates a CanvasLayer node for HUDs and overlays. Use to layer UI above the game world.',
          inputSchema: {
            type: 'object',
            properties: {
              projectPath: { type: 'string', description: 'Absolute path to project directory containing project.godot. Use the same path across all tool calls in a workflow.' },
              scenePath: { type: 'string', description: 'Path to the scene file' },
              parentNodePath: { type: 'string', description: 'Path to the parent node' },
              nodeName: { type: 'string', description: 'Name for the CanvasLayer node' },
              layerIndex: { type: 'number', description: 'Layer order (higher draws on top)' },
              followViewport: { type: 'boolean', description: 'Whether the layer follows the viewport' },
            },
            required: ['projectPath', 'scenePath', 'parentNodePath'],
          },
        },
        {
          name: 'setup_parallax_background',
          description: 'Creates a ParallaxBackground with multiple ParallaxLayer nodes. Use for scrolling backgrounds with depth.',
          inputSchema: {
            type: 'object',
            properties: {
              projectPath: { type: 'string', description: 'Absolute path to project directory containing project.godot. Use the same path across all tool calls in a workflow.' },
              ...
              layers: {
                ...
                items: {
                  ...
                  properties: {
                    name: { type: 'string' },
                    texture: { type: 'string' },
                    motionScale: { type: 'object', properties: { x: { type: 'number' }, y: { type: 'number' } } },
                    position: { type: 'object', properties: { x: { type: 'number' }, y: { type: 'number' } } },
                    scale: { type: 'object', properties: { x: { type: 'number' }, y: { type: 'number' } } },
                  },
                },
                description: 'Array of parallax layer definitions',
              },
            },
            required: ['projectPath', 'scenePath', 'parentNodePath', 'layers'],
          },
        },
        {
          name: 'add_area_2d',
          description: 'Creates an Area2D with a CollisionShape2D. Use for trigger zones and detection areas.',
          inputSchema: {
            type: 'object',
            properties: {
              projectPath: { type: 'string', description: 'Absolute path to project directory containing project.godot. Use the same path across all tool calls in a workflow.' },
              scenePath: { type: 'string', description: 'Path to the scene file' },
              parentNodePath: { type: 'string', description: 'Path to the parent node' },
              nodeName: { type: 'string', description: 'Name for the Area2D node' },
              shape: { type: 'string', enum: ['rectangle', 'circle', 'capsule'], description: 'Type of collision shape' },
              size: { type: 'object', properties: { x: { type: 'number' }, y: { type: 'number' }, radius: { type: 'number' } }, description: 'Shape dimensions' },
              layers: { type: 'number', description: 'Collision layer bitmask' },
              monitorable: { type: 'boolean', description: 'Whether other areas can monitor this' },
            },
            required: ['projectPath', 'scenePath', 'parentNodePath', 'nodeName'],
          },
        },
        {
          name: 'setup_character_body_2d',
          description: 'Creates a CharacterBody2D with collision shape, optional sprite, and optional movement script. Use for player-controlled 2D characters.',
          inputSchema: {
            type: 'object',
            properties: {
              projectPath: { type: 'string', description: 'Absolute path to project directory containing project.godot. Use the same path across all tool calls in a workflow.' },
              scenePath: { type: 'string', description: 'Path to the scene file' },
              parentNodePath: { type: 'string', description: 'Path to the parent node' },
              nodeName: { type: 'string', description: 'Name for the CharacterBody2D node' },
              shape: { type: 'string', enum: ['rectangle', 'circle', 'capsule'], description: 'Type of collision shape' },
              size: { type: 'object', properties: { x: { type: 'number' }, y: { type: 'number' }, radius: { type: 'number' } }, description: 'Shape dimensions' },
              sprite: { type: 'string', description: 'Resource path to texture for the sprite' },
              script: { type: 'string', enum: ['platformer', 'top_down', 'none'], description: 'Movement script template to attach' },
            },
            required: ['projectPath', 'scenePath', 'parentNodePath', 'nodeName'],
          },
        },
        {
          name: 'setup_static_body_2d',
          description: 'Creates a StaticBody2D with a CollisionShape2D. Use for walls, floors, and other static geometry.',
          inputSchema: {
            type: 'object',
            properties: {
              projectPath: { type: 'string', description: 'Absolute path to project directory containing project.godot. Use the same path across all tool calls in a workflow.' },
              scenePath: { type: 'string', description: 'Path to the scene file' },
              parentNodePath: { type: 'string', description: 'Path to the parent node' },
              nodeName: { type: 'string', description: 'Name for the StaticBody2D node' },
              shape: { type: 'string', enum: ['rectangle', 'circle', 'capsule'], description: 'Type of collision shape' },
              size: { type: 'object', properties: { x: { type: 'number' }, y: { type: 'number' }, radius: { type: 'number' } }, description: 'Shape dimensions' },
              layers: { type: 'number', description: 'Collision layer bitmask' },
            },
        ...
        {
          name: 'add_y_sort_container',
          description: 'Creates a Node2D with YSort functionality for top-down depth sorting. Use for top-down games.',
          inputSchema: {
            type: 'object',
            properties: {
              projectPath: { type: 'string', description: 'Absolute path to project directory containing project.godot. Use the same path across all tool calls in a workflow.' },
              scenePath: { type: 'string', description: 'Path to the scene file' },
              parentNodePath: { type: 'string', description: 'Path to the parent node' },
              nodeName: { type: 'string', description: 'Name for the YSort node' },
            },
            required: ['projectPath', 'scenePath', 'parentNodePath'],
          },
        },
        {
          name: 'set_node_2d_transform',
          description: 'Sets position, rotation, or scale on a 2D node. Convenience shortcut over set_node_properties for Vector2 types.',
          inputSchema: {
            type: 'object',
            properties: {
              projectPath: { type: 'string', description: 'Absolute path to project directory containing project.godot. Use the same path across all tool calls in a workflow.' },
              ...
              position: { type: 'object', properties: { x: { type: 'number' }, y: { type: 'number' } }, description: 'Position as Vector2' },
              rotation: { type: 'number', description: 'Rotation in degrees' },
              scale: { type: 'object', properties: { x: { type: 'number' }, y: { type: 'number' } }, description: 'Scale as Vector2' },
            },
            required: ['projectPath', 'scenePath', 'nodePath'],
          },
        },
        {
          name: 'add_path_2d',
          description: 'Creates a Path2D with a Curve2D. Use for navigation paths and follow curves.',
          inputSchema: {
            type: 'object',
            properties: {
              projectPath: { type: 'string', description: 'Absolute path to project directory containing project.godot. Use the same path across all tool calls in a workflow.' },
              scenePath: { type: 'string', description: 'Path to the scene file' },
              parentNodePath: { type: 'string', description: 'Path to the parent node' },
              nodeName: { type: 'string', description: 'Name for the Path2D node' },
              points: { type: 'array', items: { type: 'object', properties: { x: { type: 'number' }, y: { type: 'number' } } }, description: 'Array of Vector2 points' },
