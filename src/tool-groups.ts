...
export const TOOL_GROUPS: Record<string, ToolGroupDefinition> = {
  ...
  testing: {
    ...
    keywords: ['screenshot', 'capture', 'inject input', 'simulate', 'test input', 'inject key', 'inject mouse', 'viewport capture', 'screen capture', 'click', 'press key', 'mouse click', 'mouse motion'],
  },
  runtime_test: {
    description: 'Closed-loop runtime testing: scenario runner, assertions, screenshot diff, recordings, perf monitors, stress test, persisted reports',
    tools: [
      'wait_for_node',
      'monitor_properties',
    ...
    keywords: [
      'test scenario', 'run test', 'assert', 'assertion', 'pass fail', 'wait for node',
      'screenshot diff', 'image diff', 'compare screenshots', 'compare images', 'image match',
      'find button', 'click button', 'find ui', 'ui element', 'label text', 'screen text', 'visible text',
      'monitor property', 'batch properties', 'capture frames', 'frame sequence',
      'record input', 'replay recording', 'replay input', 'record macro',
      'stress test', 'fuzz', 'fuzzer', 'random input', 'soak test',
      'test report', 'test run', 'test results', 'pass fail report',
      'performance monitor', 'perf metric', 'fps monitor',
      'closed loop', 'autonomous test', 'integration test', 'end to end',
    ],
  },
  dx_tools: {
